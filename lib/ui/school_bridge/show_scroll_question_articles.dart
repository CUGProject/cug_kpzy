...
class _FollowState extends State<Follow> {
  ...
  _getArticals() async
  {
    ...
    await http.post("http://www.cugkpzy.com/get_gaoxiao_hot_questions_and_its_response_total").then(
            (req)
            {
              map = jsonDecode(req.body);
            }
    );
    print(map["host_articles"]);
    for(int i=0;i<map["hot_articles"].length;i++)
    {
      widget.articleList[i].headUrl = "https://ss1.bdstatic.com/70cFvXSh_Q1YnxGkpoWK1HF6hhy/it/u=2174909441,2495215020&fm=26&gp=0.jpg";
      widget.articleList[i].title = map["hot_articles"][i]["title"];
      widget.articleList[i].agreeNum = 0;
      widget.articleList[i].commentNum = map["hot_articles"][i]["response_num"];
      widget.articleList[i].money = int.parse(map["hot_articles"][i]["give_money_amount"]);
    }
  }
  @override
  void initState()
